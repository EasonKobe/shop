...
define(function (require) {
    ...
    Tansun.controller('roleTreeCtrl', function ($scope, $rootScope, jfRest, $http, jfGlobal, $location, jfLayer, $timeout) {
        $scope.clntendId = '10002';
        $scope.menuClick = function (clntendId) {
            $scope.clntendId = clntendId;
        ...
        $scope.userList = {
            ...
            params: {clntendId: '10002'}, //表格查询时的参数信息
            paging: true,//默认true,是否分页
            resource: 'roleUser.query',//列表的资源
            callback: function () { //表格查询后的回调函数
            }
        };
        $scope.queryMenuEventParam = {clntendId: '10002'}
        //资源列表
        ...
        //角色树
        $scope.roleQuery = {clntendId: '10002'};
        $scope.radioRole = {
            isCheck: false,
            resource: 'role.query',
            params: $scope.roleQuery,
            nodeClick: function (treeNode) {
                $scope.nodeClick(treeNode);
            }
        };
        $scope.roleFrontQuery = {clntendId: '10001'};
        $scope.radioRoleFront = {
            isCheck: false,
            resource: 'role.query',
            params: $scope.roleFrontQuery,
            nodeClick: function (treeNode) {
                $scope.nodeClick(treeNode);
            }
        };
        ...
        $scope.nodeClick = function (treeNode) {
            jfRest.request('role', 'get', Tansun.param({'roleCode': treeNode.id})).then(function (data) {
                if (data.status == 200) {
                    ...
                    $scope.queryMenu.clntendId = $scope.clntendId;
                    $scope.menu.reload();
                    //刷新菜单的按钮事件
                    $scope.queryMenuEventParam.roleCode = $scope.role.roleCode;
                    $scope.queryMenuEventParam.clntendId = $scope.clntendId;
                    $scope.eventResList.search();
                    //4.刷新已分配菜单树结构及 事件数据权限列表的
                    //刷新已分配菜单树结构
//					$scope.menuAssignedReloadTree();
                    ...
                    $scope.userList.params.clntendId = $scope.clntendId;
...
//			$validation.validate($parse('roleForm')($scope)).success(function(){//校验成功后, 进行保存操作 
            $scope.role2.clntendId = $scope.clntendId;
...
//		  });
        ...
        $scope.reloadRoleTree = function () {
            var guid = Tansun.GUID();
            $scope.radioRole.reload();
            $scope.radioRoleFront.reload();
            $scope.role = {};
            $scope.userList.params = {clntendId: $scope.clntendId};
            $scope.userList.search();

            $scope.queryMenu.roleCode = guid;
            $scope.menu.reload();

            $scope.queryMenuEventParam.roleCode = guid;
            $scope.queryMenuEventParam.menuCodes = '-1';
            $scope.eventResList.search();
